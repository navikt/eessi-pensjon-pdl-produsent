...
class P8000AndP10000Ident(): AbstractIdent() {
    ...
    override fun hentRelasjoner(sed: SED): List<Fodselsnummer?> {
        ...
        hentAnnenpersonRelasjon(sed).let { fnrListe.add(forsikret) }
        fnrListe.add(forsikret)
    ...
    fun hentAnnenpersonRelasjon(sed: SED): Fodselsnummer? {
            ...
            annenPerson?.let { person ->
                ...
                val annenPersonRelasjon = when (person.rolle) {
                    ...
                    BARN.kode, FORSORGER.kode, ETTERLATTE.kode -> Fodselsnummer.fra(
                        annenPersonPin?.value
                    )
                    else -> null
                }
