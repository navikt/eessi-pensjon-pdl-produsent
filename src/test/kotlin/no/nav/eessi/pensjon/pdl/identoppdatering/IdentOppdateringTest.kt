...
import no.nav.eessi.pensjon.pdl.identoppdatering.TestDataPDL.sedHendelse
...
internal class IdentOppdateringTest {
    ...
    fun `Gitt at SEDen inneholder to danske på som varierer i format så skal det validere og oppdatere`() {
        ...
        assertEquals(
            ...
            (identoppdatering.oppdaterUtenlandskIdent(sedHendelse(avsenderLand = "DK")))
        )
    }
    ...
    fun `Gitt at vi har en SED med norsk fnr som skal oppdateres til pdl, der PDL har en aktoerid inne i PDL saa skal Oppdateringsmeldingen til PDL ha norsk FNR og ikke aktoerid`() {
        val identifisertPerson= TestDataPDL.identifisertPerson(uidFraPdl = listOf(TestDataPDL.utenlandskIdentifikasjonsnummer(SVENSK_FNR)))

        every { personService.hentPerson(NorskIdent(FNR)) } returns
                TestDataPDL.personFraPDL(id = FNR).copy(identer = listOf(IdentInformasjon("1234567891234", IdentGruppe.AKTORID), IdentInformasjon(FNR, IdentGruppe.FOLKEREGISTERIDENT)))
                        .copy(utenlandskIdentifikasjonsnummer = listOf(TestDataPDL.utenlandskIdentifikasjonsnummer(FINSK_FNR).copy(utstederland = "FIN")))
